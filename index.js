...
app.post('/webhook/email', async (req, res) => {
  try {
    ...
    const { headers, envelope, plain, html } = email;

    // 📥 Normalisation des champs pour ta table mails
    const newMail = {
      user_id: null,                       // Remplacer par ID utilisateur si disponible
...
app.post('/assistant/message', async (req, res) => {
  try {
    ...
    if (process.env.N8N_WEBHOOK) {
      // 🔄 Envoi à n8n
      const n8nRes = await fetch(process.env.N8N_WEBHOOK, {
        method: 'POST',
